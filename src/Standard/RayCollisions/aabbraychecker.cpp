...
gly_float AABBRayChecker::AllowedHorizontalMove(const gly_float& attempt, RayCollisionGroup* group) const {
    ...
    if(attempt > 0){
        ss::vec2<gly_float> br = rect.GetCorner(ss::CORNER_BOTTOM_RIGHT);
        std::list<ss::vec2<gly_float>> l = ss::PointsBetween(br, br+ss::vec2<gly_float>(0.0f,rect.dimensions.y), horizontal_resolution);
        for(auto iter = l.begin(); iter != l.end(); iter++){
            curr_mag = group->HorizontalCollide(ss::HorizontalDirRay<gly_float>(*iter, curr_mag));
        }
        return curr_mag;
    }
    else{ //attempt < 0
        ss::vec2<gly_float> bl = rect.GetCorner(ss::CORNER_BOTTOM_LEFT);
        std::list<ss::vec2<gly_float>> l = ss::PointsBetween(bl, bl+ss::vec2<gly_float>(0.0f,rect.dimensions.y), horizontal_resolution);
        for(auto iter = l.begin(); iter != l.end(); iter++){
            curr_mag = group->HorizontalCollide(ss::HorizontalDirRay<gly_float>(*iter, -curr_mag));
        }
        return -curr_mag;
